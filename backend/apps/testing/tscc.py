...
def create_tscc_test_items(test_id):
    create_test_items(test_id, tscc_items)


def calculate_und(items):
...
def calculate_tscc_test_scores(test):
    raw_scores = calculate_raw_scores(test)
    raw_scores['UND'] = calculate_und(test.items)
    raw_scores['HYP'] = calculate_hyp(test.items)
    return_obj = convert_to_return_value(raw_scores, tscc_results_order, test)
    return return_obj
